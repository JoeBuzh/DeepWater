...
def wipe_anomaly(data: pd.DataFrame) -> pd.DataFrame:
    """
    根据每个col的分布，其分布5%以下、95%以上的数据视为异常，重置为nan
    """
    tmp = data[data.columns]

    for i, col in enumerate(tmp.columns[1:]):
        tmp.loc[tmp[col]<tmp[col].quantile(.03), col] = np.nan
        tmp.loc[tmp[col]>tmp[col].quantile(.97), col] = np.nan
